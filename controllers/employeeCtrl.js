...
const employeeCtrl = {
    register: async (req, res) => {
        try {
            const {fullName, designation, email, phone, dateOfBirth, gender, emergencyPhone, hireDate, dutyType, userName, password} = req.body;

            const userEmail = await Employees.findOne({email})
            if(userEmail) return res.status(400).json({msg: "The email already exists."})

            const user_Name = await Employees.findOne({userName})
